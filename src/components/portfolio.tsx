...
export interface YourPortfoProps {
  formData: {
    firstYear: string;
    lastYear: string;
    purpose: string;
...
const YourPortfo: React.FC<YourPortfoProps> = ({ formData, onInputChange }) => {
  ...
  const selectedWorkingGraphicsOption = workingGraphicsOptions.find(
    ...
    <FormItem htmlFor="firstYear" top="Год поступления в колледж">
      <Input type="number" id="firstYear" value={formData.firstYear} onChange={(e) => onInputChange('firstYear', e.target.value)} name="firstYear" required/>
    </FormItem>

    <FormItem htmlFor="lastYear" top="Год выпуска из колледжа">
      <Input id="lastYear" type="number" value={formData.lastYear} onChange={(e) => onInputChange('lastYear', e.target.value)} name="lastYear" required/>
    </FormItem>
    <FormItem
        top="Факультет"
